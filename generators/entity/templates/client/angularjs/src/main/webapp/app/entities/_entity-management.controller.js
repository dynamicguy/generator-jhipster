...
(function() {
    ...
    <%= entityAngularName %>Controller.$inject = [<% if (pagination === 'pagination' || pagination === 'pager') { %>'$state', <% } %><% if (fieldsContainBlob) { %>'DataUtils', <% } %>'<%= entityClass %>'<% if (searchEngine === 'elasticsearch' || searchEngine === 'solr') { %>, '<%= entityClass %>Search'<% } %><% if (pagination !== 'no') { %>, 'ParseLinks', 'AlertService', 'paginationConstants'<% } %> <%_ if (pagination === 'pager' || pagination === 'pagination'){ %>, 'pagingParams'<% } %>];

    function <%= entityAngularName %>Controller(<% if (pagination === 'pagination' || pagination === 'pager') { %>$state, <% } %><% if (fieldsContainBlob) { %>DataUtils, <% } %><%= entityClass %><% if (searchEngine === 'elasticsearch' || searchEngine === 'solr') { %>, <%= entityClass %>Search<% } %><% if (pagination !== 'no') { %>, ParseLinks, AlertService, paginationConstants<% } %> <%_ if (pagination === 'pager' || pagination === 'pagination'){ %>, pagingParams<% } %>) {
